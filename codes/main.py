...
def parse_args():
    ...
    parser.add_argument('--consistency_mode', type=str, default='dual', 
                        choices=['dual', 'prior_only', 'feature_only'],
                        help='Ablation study mode: dual (default), prior_only, or feature_only')
    parser.add_argument('--mu', type=float, default=0.1, 
                        help='Balance factor for fedprox')
    # 6. 测试伪标记性能试验
    parser.add_argument('--eval_pseudo', action='store_true', 
                        help='开启追踪 U 集合伪标签质量的实验')

        # 6. 正式阶段伪标签刷新周期
    parser.add_argument('--pseudo_refresh_gap', type=int, default=10,
                        help='Refresh gap for cached pseudo-labels after warm-up.')

    # 7. 构造 client-specific teacher 时保留的 top-m 相似客户端数量
    parser.add_argument('--teacher_top_m', type=int, default=5,
                        help='Number of most similar clients used to build each client-specific teacher.')

    # 8. teacher 权重三项因素的指数超参数
    parser.add_argument('--teacher_alpha', type=float, default=1.0,
                        help='Exponent for the client sample-size factor in teacher weighting.')
    parser.add_argument('--teacher_beta', type=float, default=1.0,
                        help='Exponent for the positive-prior factor in teacher weighting.')
    parser.add_argument('--teacher_gamma', type=float, default=1.0,
                        help='Exponent for the U-distribution similarity factor in teacher weighting.')
...
def main(args=None):
    ...
    args.arch = getattr(model.nets_list[0], 'name', model.nets_list[0].__class__.__name__)

        ...
        if self.args.averaing == 'weight':
            ...
            for dl in online_clients_dl:
                if hasattr(dl.sampler, 'indices'):
                    # 兼容原版 RevisitFL 的 SubsetRandomSampler
                    online_clients_len.append(len(dl.sampler.indices))
                else:
                    # 兼容我们 FedPU 的 RandomSampler / PUDatasetWrapper
                    online_clients_len.append(len(dl.dataset))
